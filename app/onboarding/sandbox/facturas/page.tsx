...
function KPICard({ label, valor, sub, accent }: { label: string; valor: string | number; sub: string; accent: string }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderTop: `3px solid ${accent}`, borderRadius: 4, padding: "14px 16px" }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.06em", fontFamily: sans, marginBottom: 8 }}>{label}</div>
      <div style={{ fontFamily: mono, fontSize: 19, fontWeight: 700, color: "#111", marginBottom: 2 }}>{valor}</div>
      <div style={{ fontSize: 11, color: "#9ca3af", fontFamily: sans }}>{sub}</div>
    </div>
  );
}

export default function SandboxFacturasPage() {
  ...
  return (
    <div className="fade-in" data-tour="page-facturas">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: serif, fontSize: 22, fontWeight: 700, color: "#111", marginBottom: 2 }}>Facturas</h1>
          <div style={{ fontSize: 13, color: "#6b7280", fontFamily: sans }}>Comprobantes fiscales electrónicos de prueba</div>
        </div>
        <button onClick={() => setModalAbierto(true)} disabled={productos.length === 0} data-tour="btn-nueva-factura"
          style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 18px", background: productos.length === 0 ? "#9ca3af" : "#111", color: "#fff", border: "none", borderRadius: 4, cursor: productos.length === 0 ? "not-allowed" : "pointer", fontSize: 13, fontWeight: 500, fontFamily: sans }}>
          <Icon name="plus" size={14} /> Nueva Factura
        </button>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 20 }}>
        <KPICard label="Cobrado"   valor={"RD$ " + fmt(totalCobrado)}    sub="Facturas pagadas"    accent="#0e7490" />
        <KPICard label="Por cobrar" valor={"RD$ " + fmt(totalPendiente)} sub="Facturas pendientes" accent="#92400e" />
        <KPICard label="Anuladas"  valor={anuladas}                       sub="Comprobantes"        accent="#991b1b" />
      </div>
      ...
      {productos.length === 0 && (
        <div style={{ padding: "10px 14px", background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 4, fontSize: 12, color: "#92400e", fontFamily: sans, marginBottom: 16 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: "center", padding: "60px 0", fontFamily: sans }}>
          <div style={{ fontSize: 16, color: "#374151", fontWeight: 600, marginBottom: 6 }}>{facturas.length === 0 ? "No hay facturas aún" : "Sin resultados"}</div>
          <div style={{ fontSize: 13, color: "#9ca3af" }}>{facturas.length === 0 ? "Emite tu primera factura de prueba" : "Ajusta los filtros"}</div>
        </div>
      ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#f9fafb", borderBottom: "2px solid #e5e7eb" }}>
                {["e-CF", "Tipo", "Fecha", "Cliente", "Total", "Estado", "DGII (demo)", ""].map((h) => (
              ...
              {filtradas.map((f) => {
                ...
                return (
                  <tr key={f.id} style={{ borderBottom: "1px solid #f3f4f6" }}
                    ...
                    <td style={{ padding: "11px 14px", fontFamily: mono, fontSize: 11, color: "#6b7280" }}>{f.tipoECF}</td>
                    ...
                    <td style={{ padding: "11px 14px", fontFamily: mono, fontSize: 13, fontWeight: 700, color: "#111" }}>RD$ {fmt(calcTotales(f.items).total)}</td>
                    <td style={{ padding: "11px 14px" }}><Badge estado={f.estado} /></td>
                    <td style={{ padding: "11px 14px" }}>{f.estadoDGII === "Aceptado" ? <Badge tipo="success">Aceptado</Badge> : <Badge tipo="neutral">—</Badge>}</td>
                    <td style={{ padding: "11px 14px" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => setImprimiendo(f)} style={{ padding: "5px 10px", background: "#fff", border: "1px solid #d1d5db", borderRadius: 4, cursor: "pointer", color: "#374151", display: "flex", alignItems: "center", gap: 4, fontSize: 12, fontFamily: sans }}>
                          <Icon name="print" size={12} /> Imprimir
                        </button>
                        {f.estado !== "anulada" && (
                          <button onClick={() => cambiarEstado(f.id, "anulada")} style={{ padding: "5px 10px", background: "#fff", border: "1px solid #fecaca", borderRadius: 4, cursor: "pointer", color: "#dc2626", fontSize: 12, fontFamily: sans }}>
                            Anular
                          </button>
                        )}
                      </div>
      ...
      <ModalNuevaFactura
        ...
        clientes={clientes} productos={productos} nombreEmisor={tenant.nombreNegocio} rncEmisor={tenant.rnc} rootDomain={ROOT_DOMAIN}
        onSave={onSave}
      />
