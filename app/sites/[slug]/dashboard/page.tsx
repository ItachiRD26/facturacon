...
export default function TenantDashboardPage() {
  ...
  const { productos,    loading: lp  } = useProductos(tenant.tenantId);
  const { cuentas,      loading: lcu } = useCuentasPorCobrar(tenant.tenantId);
    ...
    <div className="fade-in">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: "#6b7280", fontFamily: sans }}>{tenant.nombreNegocio} · RNC {tenant.rnc}</div>
        </div>
      ...
      {tenant.otrasEmpresas.length > 0 && (
        <div style={{ marginTop: 24, fontSize: 12, color: "var(--c-text-3)", fontFamily: sans }}>
          Otras empresas que administras:{" "}
          {tenant.otrasEmpresas.map((t, i) => (
            <span key={t.tenantId}>
              {i > 0 && ", "}
              <a href={`https://${t.slug}.${process.env.NEXT_PUBLIC_ROOT_DOMAIN}/dashboard`} style={{ color: "var(--c-brand)", fontWeight: 600 }}>
                {t.nombreNegocio}
              </a>
            </span>
          ))}
