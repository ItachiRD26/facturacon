...
export default function ClientesPage() {
  ...
  const { clientes, loading, error, agregar, actualizar, eliminar } = useClientes(tenant.tenantId);
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 4, overflow: "hidden" }}>
          ...
          {filtered.length === 0 && !loading && (
            <div style={{ padding: 32, textAlign: "center", color: "#9ca3af", fontFamily: sans, fontSize: 13 }}>
              {search || tipoFiltro !== "todos" ? "No se encontraron clientes con ese filtro" : "Agrega tu primer cliente para empezar a facturar"}
            </div>
          )}
        </div>
      )}
